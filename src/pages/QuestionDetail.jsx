...
export default function QuestionDetail() {
  ...
  async function fetchData() {
    ...
    setUserPred(myPred)
    if (myPred) { setSelectedOptionId(myPred.selected_option); setStake(myPred.stake) }
    setLoading(false)
  }

  async function handlePredict() {
    if (!selectedOptionId) { setPredError('Bir seçenek seçin.'); return }
    setSubmitting(true)
    setPredError('')
    if (editing && userPred) {
      const { error } = await supabase.from('predictions').update({ selected_option: selectedOptionId, stake }).eq('id', userPred.id)
      if (error) { setPredError(error.message); setSubmitting(false); return }
      setEditing(false)
    } else {
      const { error } = await supabase.from('predictions').insert({ question_id: id, user_id: user.id, selected_option: selectedOptionId, stake })
      if (error) { setPredError(error.message); setSubmitting(false); return }
    }
    await fetchData()
    setSubmitting(false)
  ...
  async function handleResolve(optionId) {
    setResolving(true)
    const { error } = await supabase.rpc('resolve_question', { q_id: id, correct_opt_id: optionId })
  ...
  async function handleDelete() {
    ...
    for (const p of resolvedPreds) {
      ...
      if (profile) await supabase.from('profiles').update({ total_points: profile.total_points - p.points_result }).eq('id', p.user_id)
    }
  ...
  function getOptionPct(optId) {
    if (!predictions.length) return 0
    const count = predictions.filter(p => p.selected_option === optId).length
    return Math.round((count / predictions.length) * 100)
  }

  function getOptionLabel(optId) {
    return qOptions.find(o => o.id === optId)?.label || optId
  }

  function getSelectedOdds() {
    return qOptions.find(o => o.id === selectedOptionId)?.odds || 1
  }
    ...
    <div className="page-wrap">
      ...
      <div className="qd-card card">
        ...
        <div className="detail-tabs">
          <button className={`detail-tab ${activeTab === 'tahmin' ? 'active' : ''}`} onClick={() => setActiveTab('tahmin')}>Tahmin</button>
          {isCreator && (
            <button className={`detail-tab admin-tab ${activeTab === 'admin' ? 'active' : ''}`} onClick={() => setActiveTab('admin')}>Admin</button>
          )}
        </div>
        ...
        {activeTab === 'tahmin' && (
          <>
            <div className="qd-options">
              {qOptions.map(opt => {
                const pct = getOptionPct(opt.id)
                const isCorrect = question.is_resolved && question.correct_option === opt.id
                const isUserChoice = userPred?.selected_option === opt.id
                return (
                  <div
                    key={opt.id}
                    className={`qd-option ${isCorrect ? 'correct' : ''} ${isUserChoice && !editing ? 'user-choice' : ''} ${showPredictForm ? 'selectable' : ''} ${selectedOptionId === opt.id && showPredictForm ? 'selected' : ''}`}
                    onClick={() => { if (showPredictForm) setSelectedOptionId(opt.id) }}
                  >
                    ...
                    <div className="qd-option-content">
                      ...
                      <div className="qd-option-right">
                        <span className="qd-odds mono">{opt.odds}×</span>
            ...
            {showPredictForm && (
              <div className="predict-section">
                ...
                <p className="stake-hint mono">
                  {selectedOptionId
                    ? `Doğru → +${Math.round(stake * getSelectedOdds())} puan · Yanlış → −${stake} puan`
                    : 'Önce bir seçenek seçin'}
                </p>
                {predError && <p className="error-msg">{predError}</p>}
                <div style={{display:'flex', gap:'10px', marginTop:'12px'}}>
                  <button className="btn-primary" onClick={handlePredict} disabled={submitting || !selectedOptionId}>
                    {submitting ? 'Kaydediliyor...' : editing ? 'Tahmini Güncelle' : 'Tahmini Kaydet'}
                  </button>
                  {editing && (
                    <button className="btn-ghost" onClick={() => { setEditing(false); setSelectedOptionId(userPred.selected_option); setStake(userPred.stake) }}>İptal</button>
                  )}
                </div>
              </div>
            )}
            ...
            {userPred && !editing && (
              <div className={`user-result ${userPred.points_result === null ? 'pending' : userPred.points_result >= 0 ? 'win' : 'lose'}`}>
                <div className="user-result-left">
                  ...
                  <span className="user-result-choice">{getOptionLabel(userPred.selected_option)}</span>
                  <span className="user-result-points mono">
                    {userPred.points_result === null
                      ? `Bahis: ${userPred.stake} · Sonuç bekleniyor`
        ...
        {activeTab === 'admin' && isCreator && (
          <div className="admin-tab-content">
            {!question.is_resolved && (
              <div className="admin-section">
                ...
                <div className="resolve-btns" style={{marginTop:'14px'}}>
                  {qOptions.map(opt => (
                    <button key={opt.id} className="resolve-opt-btn" onClick={() => handleResolve(opt.id)} disabled={resolving}>
                      ✓ {opt.label} <span className="mono" style={{color:'var(--accent)', marginLeft:'6px'}}>{opt.odds}×</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {question.is_resolved && (
              <div className="admin-section">
                <p className="resolve-hint" style={{color:'var(--accent)'}}>
                  ✓ Sonuçlandı. Doğru cevap: <strong>{getOptionLabel(question.correct_option)}</strong>
                </p>
              </div>
            )}
            ...
            <div className="admin-section admin-delete">
              ...
              <p style={{fontSize:'0.85rem', color:'var(--text-muted)', marginBottom:'12px'}}>Bu soru ve tüm tahminler kalıcı olarak silinir. Verilen puanlar geri alınır.</p>
              {!confirmDelete ? (
                ...
                <div className="delete-confirm">
                  ...
                  <div style={{display:'flex', gap:'10px', marginTop:'12px'}}>
                    <button className="btn-danger" onClick={handleDelete} disabled={deleting}>{deleting ? 'Siliniyor...' : 'Evet, Sil'}</button>
          ...
          <div className="preds-table">
            ...
            {predictions.map(p => (
              <div key={p.id} className="pred-row">
                ...
                <span className="pred-choice">{getOptionLabel(p.selected_option)}</span>
